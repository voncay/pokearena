...
const [ennemyTeamStatus,setennemyTeamStatus] = useState([1,1,1])
  ...
  const badgesCondition = (initialValueBadges) => badges.reduce(
    (accumulator, currentValue) => accumulator + currentValue,
    initialValueBadges
  )

 
  const aliveCondition = (initialValueTeam) => {initialValueTeam.reduce(
    (accumulator, currentValue) => accumulator + currentValue,
    initialValueTeam
  )}
...
const pokemonHealthAlly = (num) => {
  ...
  const [battleTeam, setBattleTeam] = useState({
    pokemon1: {
      pokemonImg:
        playerTeam[0].sprites.back_default,
  ...
  return (
    <div className="brockpage">
      <div className="brockarenaframe">
        ...
        <h1 className="battlefont"> {battleTeam.pokemon1.pokemonAttacks.attackOne.name} </h1>
      </div>
    </div>
  );
};
